...
**另有一套双实例联机验收**（`tests/net-qa.js`，30 项断言），开两个标签页跑通整条联机路径：
...
**应战码只允许成功应用一次**，这是两条断言专门盯着的。打洞要好几秒且界面在这期间没有明显变化，用户很容易再点一次「应用应战码」；而第一次成功后 `signalingState` 已经回到 `stable`，再调 `setRemoteDescription` 会抛 `Called in wrong state: stable`——浏览器的原文，用户既看不懂也不知道下一步做什么。所以面板在成功后会一直禁用该按钮（`net-panel.js` 的 `answerApplied`），`RtcTransport.acceptAnswer()` 里另有一道状态校验作为第二防线。**注意只有成功才上锁**：应战码有误时 `setRemoteDescription` 会拒绝且状态不变，那种情况必须让用户改完再试一次，所以按钮不能提前锁死。
