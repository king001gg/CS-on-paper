...
| B2. 双实例联机 | `node tests/net-qa.js` | 30 项断言：两个标签页建链、邀请码容错、面板全流程、应战码重复应用 | 跨机器 / 跨防火墙的真实连通性 | ~1 分钟 |
...
联机那 30 项同理，而且它的边界更值得说清楚：`net-qa.js` 跑的是**同一台机器上的两个标签页**，
...
- `tests/net-qa.js` —— 双标签页联机验收，493 行。**它跟 `browser-qa.js` 的 CDP 接法不一样**
